...
int sudoku_candidatos_c(CELDA cuadricula[NUM_FILAS][NUM_COLUMNAS], uint8_t fila, uint8_t columna)
{
   ...
   // Si la celda ya tiene un valor no hay que calcular candidatos.
   ...
   for (i = 0; i < NUM_FILAS; i++)
      for (j = 0; j < NUM_FILAS; j++)
		 ...
		 if (valor == ERROR) {
			...
			numCeldasVacias -= sudoku_candidatos_c(cuadricula, i, j);
		 }
...
int sudoku_recalcular_c_arm(CELDA cuadricula[NUM_FILAS][NUM_COLUMNAS])
{
   ...
   for (i = 0; i < NUM_FILAS; i++)
      for (j = 0; j < NUM_FILAS; j++)
         numCeldasVacias -= sudoku_candidatos_armU9(cuadricula, i, j);
...
int sudoku_recalcular_c_thumb(CELDA cuadricula[NUM_FILAS][NUM_COLUMNAS])
{
   ...
   for (i = 0; i < NUM_FILAS; i++)
      for (j = 0; j < NUM_FILAS; j++)
         numCeldasVacias -= sudoku_candidatos_c_thumb(cuadricula, i, j);
